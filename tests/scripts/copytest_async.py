...
async def run_tests(args: Namespace) -> tuple[list[float], ...]:
    ...
    async with await psycopg.AsyncConnection.connect(args.dsn) as conn:
        async with conn.cursor() as cur:
            ...
            for i in range(n_insert_select):
                t_insert_select = await execute_insert_select(args, test, cur)
                t_insert_selects.append(t_insert_select)
                await cur.execute(test.get_truncate_stmt())
            for i in range(n_in):
                t_in = await copy_in(args, test, cur)
                if args.copy_in:
                    t_ins.append(t_in)
                if not i == n_in - 1:
                    await cur.execute(test.get_truncate_stmt())
            ...
            for i in range(n_out):
                t_out = await copy_out(args, test, cur)
                t_outs.append(t_out)
